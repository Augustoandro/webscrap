...
function get_details($url)
{
 ...
 $sql2="select * from websid where stitle = '".$title."' limit 1";
 $rs=mysqli_query($_con,$sql2);
 if (mysqli_fetch_assoc($rs) == false) {
     $sql=$_con->prepare("insert into websid(stitle,slink,skey,sdesc,simg,styp) values(?,?,?,?,?,?)");
     $sql->bind_param('ssssss',$title,$url,$keywords,$description,$imag,$type);
     $sql->execute();       
     echo "Inputting data";
